...
当前项目处在“内部正式使用准备”阶段：先稳定无依赖 MVP 和真实使用验收，再分阶段做正式化迁移。项目总控、技术债优先级和后续决策规则见 [docs/project-control.md](docs/project-control.md)。
...
- 筛选工作台：关键词搜索能力已同步到品类可搜索多选和品牌可搜索筛选，并支持渠道、上市状态、自定义价格段、AI 置信度、自定义功能参数、产品库列排序和固定产品列；可命名保存/删除工作视图，工作视图会恢复筛选、列显隐、排序、对比型号、对比字段和路线图品牌。
