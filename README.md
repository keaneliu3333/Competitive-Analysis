...
详情页上传支持多张图片和单个 PDF。上传图片会交给 Qwen-VL 视觉识别，长图会在前端自动切片，最多发送 8 张图片；URL 预抓取会额外生成图片候选、价格候选和文案证据片段，并把最多 4 张可访问的详情页图片候选交给 Qwen-VL 理解。PDF 文件仍限制 50MB；如果只有原始 PDF 而没有页面图片，当前会进入人工复核兜底，后续可再补 PDF 转图片流程。服务端 JSON 请求体限制为 70MB，用于覆盖 base64 膨胀并拦截异常大请求。兜底结果不会保存 PDF 原文，只记录文件名、类型和大小用于追溯。
