...
const lines = [
  ...
  "- 文本型结构化抽取和型号对比总结默认走 DeepSeek；图片、长图和 URL 图片候选默认走 Qwen-VL；原始 PDF 未转图片时进入人工复核兜底。",
  "",
  "## 使用前必须通过",
  "",
  "```bash",
  ...
  "| 路线图 | Excel/SVG/PDF | 页面卡片包含产品名称、价格、Top3 卖点；导出保留正式报告字段 |",
  "| 数据包 | JSON 和 backup-before-import | 导出、导入、备份、审计和保存视图可恢复 |",
  "| 数据包与审计导出 | JSON / CSV | 完整数据包、AI 用量 CSV 和审计 CSV 可导出并可追溯 |",
...
const outputPath = join(reportDir, `formal-use-pack-${dateStamp}.md`);
writeFileSync(outputPath, lines.join("\n"), "utf8");

console.log(`Formal use pack generated: ${outputPath}`);
