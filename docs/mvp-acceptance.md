...
- 筛选：支持品类可搜索多选、品牌可搜索筛选、渠道、上市状态、自定义价格段、AI 置信度和自定义功能参数筛选。
- 自定义元素：支持新增模块和字段，字段类型包含文本、数字、布尔、枚举、价格、图片。
- 详情页分析：支持 URL 预抓取和补充说明，并通过 DeepSeek 进行文本型结构化抽取；多张图片、长图切片和 URL 图片候选通过 Qwen-VL 视觉识别；原始 PDF 未转换为图片时进入人工复核兜底。
- 型号对比：支持从已添加产品选择至少 2 个型号且不限制上限，输出参数矩阵、差异高亮和 500 字以内中文总结。
- 总结口径：总结应覆盖产品功能、关键参数、价格梯度、Top3 卖点和使用感受，不超过 500 个中文字符。
- 路线图：支持品类、品牌多选、状态、半年度周期筛选；页面路线图卡片包含产品名称、价格和 Top3 优先级卖点。
- 导出：支持产品库 Excel、型号对比 Excel、路线图 Excel、路线图 SVG、各品牌分页 PDF、数据包、CSV 模板、审计、用量、质量问题、MVP 验收清单和 Markdown 交接包导出。
- 复核与追溯：AI 结果有置信度、待确认队列、字段级确认、分析记录、价格快照、审计日志和数据质量检查。
- 内部部署与正式功能使用：支持本地持久化、读写访问令牌、OpenAI/DeepSeek/Qwen-VL 用量日志、成本估算、部署交接说明和正式功能使用运行手册/使用包生成。
...
启动本地服务后可运行 `SMOKE_BASE_URL=http://127.0.0.1:4173 node scripts/verify-formal-use-browser.mjs`，自动覆盖筛选、自定义字段、AI 导入兜底、型号对比和 500 字以内总结、路线图导出、数据包导入导出、审计/用量日志、375/768/1440 响应式视口，并生成 `reports/formal-use-browser-smoke-*.json`。
...
- 输入品类、品牌和价格段后，产品表结果随筛选变化。
- 添加一个自定义枚举字段，在产品详情中编辑后出现在筛选和对比矩阵中。
- 上传一张详情页长图，确认能进入 Qwen-VL 视觉分析流程；未配置 Qwen-VL 或上传原始 PDF 时应出现兜底结果和待确认状态。
- 选择至少 2 个型号生成 500 字以内总结，并导出对比表。
- 切换路线图筛选，分别导出 Excel、SVG 和各品牌 PDF。
