...
- AI 能力采用 provider 分层：OpenAI 负责图像/PDF/结构化抽取，DeepSeek 可用于文本总结、低成本复核和二次归纳。
- 不做公开注册、多租户计费或全网自动爬取。
...
- 指标：
  ...
  - 500 字以内总结的完整性和可读性
  - 单次分析 token 和成本
- 模型评估按 provider 记录：OpenAI 抽取、DeepSeek 总结、本地兜底分别统计成功率、错误和成本。
